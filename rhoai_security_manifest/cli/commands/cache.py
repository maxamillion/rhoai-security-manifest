...
@click.option(
    '--clean',
    is_flag=True,
    help='Clean expired cache entries'
)
@click.option(
    '--clear',
    is_flag=True,
...
@click.option(
    '--older-than',
    help='Clean entries older than specified time (e.g., 30d, 7d, 24h)'
)
@click.option(
    '--show-stats',
    is_flag=True,
...
@click.option(
    '--release',
    help='Manage cache for specific release only'
)
@click.option(
    '--vacuum',
    is_flag=True,
...
def cache(
    ...
    try:
        ...
        # Clean operations
        if clean:
            days_to_keep = _parse_time_period(older_than) if older_than else config.database.retention_days
...
def _show_cache_stats(db_manager, config) -> None:
    ...
    try:
        ...
        settings_table.add_row("Retention Period", f"{config.database.retention_days} days")
        settings_table.add_row("Cleanup Interval", f"{config.cache.cleanup_interval_hours} hours")
...
def _show_storage_recommendations(health_info: dict, config) -> None:
    ...
    if total_records > 1000:
        ...
        console.print(f"   [dim]Running cleanup could free significant space.[/dim]")
...
def _clean_cache(db_manager, days_to_keep: int, release: Optional[str]) -> None:
    ...
    if release:
        ...
        console.print(f"[yellow]Cleaning cache entries older than {days_to_keep} days...[/yellow]")
    ...
    try:
        ...
        results_table.add_row("Vulnerabilities", str(cleanup_stats["vulnerabilities_deleted"]))
...
def _clear_cache(db_manager, release: Optional[str]) -> None:
    ...
    if release:
        console.print(f"[yellow]Clearing all cache data for release {release}...[/yellow]")
...
def _vacuum_database(db_manager) -> None:
    ...
    try:
        ...
        if space_reclaimed > 0:
            console.print(f"[green]✓ Database vacuumed successfully[/green]")
...
def _parse_time_period(time_str: str) -> int:
    ...
    try:
        ...
        raise click.BadParameter(f"Invalid time period: {time_str}. Use format like '30d', '7d', '24h'")
