...
@click.option(
    '--release', 
    required=True,
    help='OpenShift AI release version (e.g., 2.8.0)'
)
@click.option(
    '--format',
...
@click.option(
    '--packages',
    is_flag=True,
    help='Include package-level vulnerability details'
)
@click.option(
    '--offline',
    is_flag=True,
    help='Use cached data only, do not make API calls'
)
@click.option(
    '--force-refresh',
    is_flag=True,
    help='Ignore cache and refresh all data from APIs'
)
@click.option(
    '--containers',
    multiple=True,
...
def generate(
    ...
    if offline and force_refresh:
        console.print("[red]Error: Cannot use --offline and --force-refresh together[/red]")
        raise click.Abort()
    ...
    try:
        ...
        with Progress(
            ...
            if output is None:
                output = _generate_output_filename(release, format, config.reports.output_directory)
...
def _convert_analysis_to_report(analysis_result, include_packages: bool, config) -> dict:
    ...
    return {
        ...
        "summary": {
            "total_vulnerabilities": sum(c["total_vulnerabilities"] for c in analysis_result.containers),
            "grade_distribution": analysis_result.summary["grade_distribution"],
            "average_vulnerabilities": round(
                sum(c["total_vulnerabilities"] for c in analysis_result.containers) / 
                len(analysis_result.containers), 1
            ) if analysis_result.containers else 0,
...
def _write_csv_report(report_data: dict, output_path: Path) -> None:
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        writer.writerow([
            ...
            'Critical', 'High', 'Medium', 'Low', 'Total Vulnerabilities'
        ])
...
def _show_summary_stats(report_data: dict) -> None:
    ...
    console.print(f"   Total Vulnerabilities: {report_data['summary']['total_vulnerabilities']}")
    console.print(f"   Average per Container: {report_data['summary']['average_vulnerabilities']}")