...
@click.option(
    '--to', 'to_release',
    required=True,
    help='Target release version to compare to'
)
@click.option(
    '--format',
...
@click.option(
    '--show-resolved',
    is_flag=True,
    help='Include vulnerabilities that were resolved'
)
@click.option(
    '--show-new',
    is_flag=True,
    help='Include new vulnerabilities found'
)
@click.option(
    '--show-common',
    is_flag=True,
...
@click.option(
    '--containers',
    multiple=True,
    help='Filter comparison to specific container names'
)
@click.pass_context
def compare(
    ctx: click.Context,
    ...
    try:
        ...
        comparison_data = _perform_comparison(
            from_data, to_data, from_release, to_release,
            show_resolved, show_new, show_common
        )
        ...
        if format == 'table' and output is None:
            ...
            if output is None:
                output = _generate_comparison_filename(from_release, to_release, format, config.reports.output_directory)
...
def _load_release_data(release: str, filter_containers: list, db_manager) -> Optional[dict]:
...
def _perform_comparison(
    ...
    for container_name in common_containers:
        ...
        comparison["summary"]["new_vulnerabilities"] += len(container_comparison["new_cves"])
        comparison["summary"]["resolved_vulnerabilities"] += len(container_comparison["resolved_cves"])
...
def _display_comparison_table(comparison_data: dict) -> None:
    ...
    summary_table.add_row("Resolved Vulnerabilities", str(summary["resolved_vulnerabilities"]))
...
def _generate_comparison_filename(from_release: str, to_release: str, format: str, output_dir: Path) -> Path:
...
def _write_comparison_report(comparison_data: dict, output_path: Path, format: str) -> None:
...
def _write_csv_comparison(comparison_data: dict, output_path: Path) -> None:
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        # Header
        writer.writerow([
            'Container', 'From Grade', 'To Grade', 'Grade Change',
            'New CVEs', 'Resolved CVEs', 'Net Change'
        ])
