...
class SecurityManifestApp:
    ...
    def _generate_manifest_workflow(self):
        ...
        include_packages = input("Include package-level details? (y/N): ").strip().lower() == 'y'
        ...
        if input("\nProceed? (Y/n): ").strip().lower() != 'n':
            ...
            console.print(f"Report saved to: security_reports/rhoai_security_manifest_{release}_TIMESTAMP.{output_format}")
        else:
    ...
    def _compare_releases_workflow(self):
        ...
        format_map = {'1': 'table', '2': 'json', '3': 'csv', '4': 'html', '5': 'markdown'}
    ...
    def _manage_cache_workflow(self):
        ...
        if choice == '1':
            ...
            days = input("Clean entries older than how many days? (default: 180): ").strip()
            days = int(days) if days.isdigit() else 180
            console.print(f"[yellow]Cleaning entries older than {days} days...[/yellow]")
            console.print("[green]✓ Cache cleaned![/green]")
        elif choice == '3':
            if input("Clear ALL cache data? This cannot be undone (y/N): ").strip().lower() == 'y':
    ...
    def _configure_settings_workflow(self):
        ...
        console.print("[dim]For now, use configuration files or environment variables.[/dim]")
