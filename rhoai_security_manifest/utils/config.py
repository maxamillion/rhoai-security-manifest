...
class DatabaseConfig(BaseModel):
    ...
    url: str = Field(default="sqlite:///security_manifest.db", description="Database connection URL")
    retention_days: int = Field(default=180, ge=30, le=365, description="Data retention period in days")
    backup_enabled: bool = Field(default=True, description="Enable automatic database backups")
    backup_retention_days: int = Field(default=30, ge=7, le=90, description="Backup retention period")
...
class APIConfig(BaseModel):
    ...
    timeout: int = Field(default=30, ge=10, le=300, description="API request timeout in seconds")
    ...
    max_concurrent_requests: int = Field(default=10, ge=1, le=50, description="Maximum concurrent API requests")
    rate_limit_per_minute: int = Field(default=100, ge=10, le=1000, description="Rate limit per minute")
...
class CacheConfig(BaseModel):
    ...
    max_size_mb: int = Field(default=500, ge=100, le=5000, description="Maximum cache size in MB")
    cleanup_interval_hours: int = Field(default=24, ge=1, le=168, description="Cache cleanup interval")


class ReportConfig(BaseModel):
    """Report generation configuration settings."""
    
    output_directory: Path = Field(default=Path("security_reports"), description="Report output directory")
    include_packages_by_default: bool = Field(default=False, description="Include package details by default")
    html_template_theme: str = Field(default="default", description="HTML report theme")
    max_report_size_mb: int = Field(default=100, ge=10, le=1000, description="Maximum report size in MB")


class SecurityConfig(BaseModel):
    """Security grading configuration settings."""
    
    use_redhat_grades: bool = Field(default=True, description="Prefer Red Hat security grades when available")
    critical_weight: int = Field(default=20, ge=1, le=50, description="Weight for critical vulnerabilities")
    high_weight: int = Field(default=10, ge=1, le=25, description="Weight for high severity vulnerabilities")
    medium_weight: int = Field(default=5, ge=1, le=15, description="Weight for medium severity vulnerabilities")
    low_weight: int = Field(default=1, ge=1, le=5, description="Weight for low severity vulnerabilities")
    unpatched_penalty: int = Field(default=10, ge=1, le=25, description="Additional penalty for unpatched vulnerabilities")
    age_factor_weight: int = Field(default=5, ge=1, le=15, description="Weight for vulnerability age factor")
...
class LoggingConfig(BaseModel):
    ...
    file_path: Path = Field(default=Path("logs/security_manifest.log"), description="Log file path")
    max_file_size_mb: int = Field(default=10, ge=1, le=100, description="Maximum log file size in MB")
    backup_count: int = Field(default=5, ge=1, le=20, description="Number of log file backups to keep")
    format: str = Field(
...
class Config(BaseModel):
    ...
    color_output: bool = Field(default=True, description="Enable colored terminal output")
