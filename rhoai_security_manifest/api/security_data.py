...
class SecurityDataClient:
    ...
    async def get_security_advisory(self, advisory_id: str) -> Optional[SecurityAdvisory]:
    ...
    async def bulk_analyze_containers(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"Failed to analyze container {containers[i]['name']}: {result}")
    ...
    async def _get_related_advisories(self, container_name: str) -> List[SecurityAdvisory]:
        """Get security advisories related to the container."""
        async with self._semaphore:
            url = urljoin(self.base_url, "security/advisory")
            
            params = {
                "q": f"openshift {container_name}",
                "limit": 50
            }
