...
class ProductListingsClient:
    ...
    async def get_openshift_ai_product(
        ...
        async with self._semaphore:
            ...
            params = {
                "filter": 'name=="Red Hat OpenShift AI"',
                "page_size": 10
            }
            ...
            try:
                ...
                logger.info(f"Found OpenShift AI product with {len(product_listing.operator_bundles)} operator bundles")
    ...
    async def extract_container_repositories(
        ...
        logger.info(f"Extracting container repositories from {len(product_listing.operator_bundles)} operator bundles")
    ...
    async def map_version_to_containers(
        ...
        logger.info(f"Mapped {len(all_repos)} containers for RHOAI version {rhoai_version}")
        return all_repos
    ...
    async def _resolve_bundle_containers(
        ...
        if bundle.package in container_mappings:
            ...
            logger.debug(f"Mapped {len(container_mappings[bundle.package])} containers for bundle {bundle.package}")
        else:
            logger.warning(f"No container mapping found for bundle package: {bundle.package}")
